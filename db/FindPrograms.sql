	...
	while @@FETCH_STATUS = 0 --the program loop
		begin
			...
			while @@FETCH_STATUS = 0 --the subject loop 
				begin
					set @e_bool = 1;
					if exists (select 'x' from EntranceRequirement ER where @programID = ER.ProgramID and @subjectID = ER.SubjectRequirementID)
						begin
							if not exists (select 'x' from EntranceRequirement ER where ER.HighSchoolCourseID in (select HSCID from CourseList) 
...
/*		
						...
						begin
							if exists (select 'x' from EntranceRequirement ER where @programID = ER.ProgramID && @subjectID = ER.SubjectRequirementID)
								begin 
									--there is an entrance requirement not met
