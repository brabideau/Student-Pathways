    ...
    <div runat="server" id="EntranceRequirements" visible="false" class="clearfix">
       ...
       <h3>High School</h3> 
                    ...
                    <asp:ListView ID="LV_EntranceReq" runat="server" DataSource='<%# Eval("EntranceReqs") %>' ItemPlaceholderID="EntrancePlaceHolder"  OnItemCommand="Ent_Req_Commands">
                        ...
                        <ItemTemplate>
                            <tr>
                                ...
                                <td runat="server">
                                    <asp:Label ID="Label4" runat="server" ><asp:TextBox ID="Ent_Marks" runat="server" Width="50px" Text='<%# Eval("Mark") %>' /> % in </asp:Label><asp:DropDownList ID="DL_HS_Course" runat="server" DataSourceID="CourseList"
        ...
        - - - - - <asp:TextBox ID="NewMark" runat="server" Width="50px" /> % in <asp:DropDownList ID="DL_New_EntReq" runat="server" DataSourceID="CourseList"
                                                ...
                                                <asp:ListItem Value="-3">Select a Course</asp:ListItem></asp:DropDownList><asp:LinkButton ID="Add_EntReq_Button" runat="server" Text="Add" CssClass="admin_button" OnClick ="Add_Ent_Req"/>
        </div>

        <hr />
        <%--POST SECONDARY ENTRANCE REQUIREMENT--%>
        <uc1:MessageUserControl runat="server" id="PSMessageUserControl" />
        <h3>Post-Secondary</h3><p>If this program requires any previous post-secondary credientials, such as a diploma or certificate, add them here.</p><div runat="server" id="PSRequirements">
            <h5>Current Entrance Requirements</h5><asp:GridView ID="GV_DegreeEntranceReq" runat="server" AutoGenerateColumns="False" ItemType="CrystalBallSystem.DAL.POCOs.GetDegEntReqs" DataKeyNames="DegreeEntranceRequirementID" OnRowDeleting="GV_DegReq_RowDeleting" OnRowEditing="GV_DegReq_RowEditing">
                <Columns>
                    ...
                    <asp:TemplateField HeaderText="Credential Name">
                        <ItemTemplate>
                            <asp:DropDownList ID="CredentialName" runat="server" DataSourceID="ODSCredentialType"
                                                DataTextField="CredentialTypeName"
                                                DataValueField="CredentialTypeID" 
                                                SelectedValue='<%# Eval("CredentialTypeID") %>' />
                    ...
                    <asp:TemplateField HeaderText="Category">
                        <ItemTemplate>
                            <asp:DropDownList ID="Category" runat="server" DataSourceID="CategoryList"
                                                DataTextField="CategoryDescription"
                                                DataValueField="CategoryID" 
                                                SelectedValue='<%# Eval("CategoryID") %>' />
                            <%--<asp:Label ID="Category" runat="server" Text='<%# Item.CategoryDescription %>' />--%>
                        </ItemTemplate>  
                    </asp:TemplateField>
                ...
                <EmptyDataTemplate>
                    No Existing Entrance Requirements.</EmptyDataTemplate></asp:GridView><h5>Add a New Entrance Requirement</h5><div runat="server" id="AddRequirements" class="clearfix">
                ...
                <p>Overall GPA: </p><asp:TextBox ID="TB_GPA" runat="server"></asp:TextBox><asp:LinkButton ID="Add_DER" runat="server" CssClass="admin_button2" OnClick="Add_DER_Click">Add Requirement</asp:LinkButton></div></div></div><%-- ----------------------------- PROGRAM COURSES ---------------------------------------%><div runat="server" id="ProgramCourses" visible="false" class="clearfix">
       ...
       <asp:ListView ID="LV_ProgramCourses_More" runat="server"
        ...
        <p>Does this program accept transfer credit/advanced credit for any other NAIT courses?</p><!--Add new equivalency to empty gridview--><asp:GridView ID="GV_Equivalencies" runat="server" AutoGenerateColumns="False"  OnRowDeleting="EquivalenciesGrid_RowDeleting" CssClass="equivalency-grid clearfix" ItemType="CrystalBallSystem.DAL.POCOs.GetEquivalencyNames" ShowFooter="True" DataKeyNames="CourseEquivalencyID">
            <Columns>
            ...
            <div class="clearfix">
                <asp:Label ID="EmptyCurrent" runat="server" Text="Program course " CssClass="col-4 label"></asp:Label><asp:DropDownList ID="EmptyCurrentDropdown" runat="server" DataSourceID="EmptyCurrentDropdownODS" 
                    ...
                    <asp:Label ID="CourseEquivalent" runat="server" Text="Course: " CssClass="col-4 label"></asp:Label><asp:DropDownList ID="EquivalentCourseID" runat="server" DataSourceID="CourseByProgramODS" DataTextField="CourseName" DataValueField="CourseID" AppendDataBoundItems="True" AutoPostBack="true" CssClass="col-5">
                        <asp:ListItem Value="-1">[Select Course]</asp:ListItem></asp:DropDownList></div><div class="col-12">
                    <asp:LinkButton ID="Enter" runat="server" OnClick="Enter_Click" CssClass="admin_button admin_button_centered submit">Add Equivalency</asp:LinkButton></div></div></div><asp:LinkButton ID="CourseEquivalencies_Save" runat="server" OnClick="Save_CourseEquivalencies" CssClass="button next button-long">Save & Continue</asp:LinkButton></div><%-- ----------------------------- PROGRAM PREFERENCES ---------------------------------------%><div runat="server" id="ProgramPreferences" visible="false" class="clearfix">
        <p>Answer the questions below so that students can be better matched with this program:</p><asp:GridView ID="GV_Questions" DataSourceID="QuestionsList" runat="server" AutoGenerateColumns="False" CssClass="prefQuestionsCSS">
            <Columns>
                <asp:TemplateField>
                    <ItemTemplate>
                        <asp:Label ID="QuestionID" runat="server" Visible="false" Text='<%# Eval("QuestionID") %>'></asp:Label></ItemTemplate></asp:TemplateField><asp:BoundField DataField="Description" HeaderText="Description" SortExpression="Description"></asp:BoundField>
                <asp:TemplateField>
                    <ItemTemplate>
                        <asp:RadioButtonList ID="RB_Preference" runat="server" RepeatLayout="OrderedList">
                            <asp:ListItem Value="1" Text="Definitely Not"></asp:ListItem><asp:ListItem Value="2" Text="No"></asp:ListItem><asp:ListItem Value="3" Text="Don't Know" Selected="true"></asp:ListItem><asp:ListItem Value="4" Text="Yes"></asp:ListItem><asp:ListItem Value="5" Text="Definitely"></asp:ListItem></asp:RadioButtonList></ItemTemplate></asp:TemplateField></Columns></asp:GridView><asp:LinkButton ID="Questions_Save" runat="server" OnClick="Save_Questions" CssClass="button next button-long">Save & Continue</asp:LinkButton></div></div><%--end program edit div--%><%-- ----------------------------- SEARCH ---------------------------------------%><hr /><div runat="server" id="search" class="search-bar" >
    ...
    <div runat="server" id="ProgramList" visible="true" class="col-12 searchbox">
        ...
        <LayoutTemplate>
            <table>
                <tr>
                    <th class="col-1"></th>
                    <th runat="server">Program Name</th><th runat="server">Description</th><th runat="server">Active?</th><th runat="server">Link</th></tr><tr id="itemPlaceholder" runat="server"></tr>
            </table>
        </LayoutTemplate>        
        <ItemTemplate>
                <tr>
